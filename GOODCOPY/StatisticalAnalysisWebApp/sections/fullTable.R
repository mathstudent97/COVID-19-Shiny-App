...
getFullTableData <- function(groupBy) {
  ...
  data <- data_evolution %>%
    ...
    add_row(
      ...
      "value_new_confirmed" = sum(.$value_new_confirmed, na.rm = T), # Sum of all confirmed values and so on.
    ...
    summarise(
      ...
      confirmed_totalNorm = round(sum(value_confirmed, na.rm = T) / max(population, na.rm = T) * 100000, 2), # 2 decimal places.
    ...
    mutate_at(vars(contains('_newPer')), list(~na_if(.,Inf))) %>% # Edge cases. In case infinity, exclude it.
    mutate_at(vars(contains('_newPer')), list(~na_if(., 0))) %>% # Also those that are 0 are not needed.
    mutate(
  ...
  select(-population) %>% # Select from population and cast it as a data frame.
  as.data.frame()
}
